...
The third prompt for [genuary](https://genuary.art) 2022 is "Space". So many interesting possibilities: outer space, orbits of celestial bodies, maps, spacing between objects (for instance using the golden ratio). For this I decided to combine some of what I learned dithering with the idea of negative space.
...
Escher mosaics are great examples of negative space. I will work with this image:

![Negative Space](mc-escher-1.jpg)

This image probably works well with divergent color palettes due to the polar nature of the work, with the two different images (birds and fish) blending into each other.
...
The image is 745 pixels in width and 739 in height. I change the colorspace to gray:
```{r}
ns <- ns %>% 
...
```{r space-escher-revolucion, cache=TRUE}
ggplot(data = img_ns %>%
...
```{r space-escher-revolucion-detail, cache=TRUE}
ggplot(data = img_ns %>%
...
```{r space-escher-revolucion-pixelated, cache=TRUE}
ggplot(data = img_ns %>%
         filter(x >= 0 & x <= 740, y >= 0 & y <= 739), 
       aes(x = x, 
           y = y, 
...
```{r space-escher-revolucion-detail-1, pixelated, cache=TRUE}
ggplot(data = img_ns %>%
         filter(x >= 195 & x <= 345, y >= 470 & y <= 590), 
       aes(x = x, 
           y = y, 
...
```{r space-escher-revolucion-detail-2, pixelated, cache=TRUE}
ggplot(data = img_ns %>%
...
It occurs to me that this image would look quite pretty with [{MetBrewer}]() palette `Isfahan1`:
...
The image is 470 pixels in width and 472 in height. I change the colorspace to gray:
```{r}
...
```{r space-escher-isfahan, cache=TRUE}
...
```{r space-escher-isfahan-detail, cache=TRUE}
ggplot(data = img_ns_knights %>%
...
José Guadalupe Posada also worked with negative space. I will try this image, that probably works best with a sequential palette:
...
The image is 400 pixels in width and 263 in height. I change the colorspace to gray:
```{r}
ns_posada <- ns_posada %>% 
  image_quantize(max = 15, 
                 colorspace = "gray",
...
```{r space-posada-tierra, cache=TRUE}
