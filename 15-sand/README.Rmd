...
For this prompt I will do something with what I learned from circle packing: packing irregular polygons, splitting polygons, using different tolerances for circles in polygons.
...
This is my improved function that allows for circle packing using radius parameters (min/max) that vary by polygon:
```{r}
...
Take the original container and pass lines through it to create a landscape by partitioning it. Create a data frame for these lines:
```{r}
...
Pack these bad boys (but use only polygons that are "ground"). Use palette `Atentado` and `Revolucion`: 
```{r}
