...
Today's prompt is to use the following colors: #2E294E, #541388, #F1E9DA, #FFD400, #D90368.

This is an excuse to do more truchet tiles. In particular I take this opportunity to add an option in package {truchet} for non-random placement of tiles in a mosaic.

Here I work with multiscale tiles using a pre-determined tile pattern as opposed to random. Load packages:
```{r}
library(ambient)
library(ggnewscale)
library(sf)
...
To create a data frame with placeholders for the tiles I first select the limits and center of the mosaic, and the radius for creating a ring:
```{r}
# Extent of mosaic
xlim <- c(0, 21)
ylim <- c(0, 21)
