...
**NOTE** These functions are now implemented in package [{truchet}](https://paezha.github.io/truchet/)
...
st_truchet_ms <- function(t1, t2, prop = 0.5, xlim = c(1, 8), ylim = c(1, 12)){
  ...
  mosaic_3 <- mosaic_2[1,] %>%
    st_difference(mosaic_2[2,]$geometry) %>%
    st_cast(to = "POLYGON") %>%
...
Additional multiscale tiles. Truchet polygons 3 (see https://christophercarlson.com/portfolio/multi-scale-truchet-patterns/):
```{r}
...
ggplot() +
  geom_sf(data = container,
          ...
          fill = "white") +
  geom_sf(data = mosaic %>%
            filter(color == 1, area > 0.28 & area < 0.2944235),
          fill = "red") +
  geom_sf(data = mosaic %>%
            filter(color == 2) %>%
            st_crop(container),
...
Truchet polygons general function (see https://christophercarlson.com/portfolio/multi-scale-truchet-patterns/):
```{r}
truchet_p <- function(scale_p = 1, type = "d"){
  # scale_p = 1 scale of tiles, can be one of c(1, 1/2)
  # See https://christophercarlson.com/portfolio/multi-scale-truchet-patterns/
  
  # Validate inputs
  assertChoice(scale_p, c(1, 1/2))
  assertChoice(type, c("d", "-", "f", "a"))
...
Create mosaic:
```{r}
# Create tiles of a single type
tiles_1 <- truchet_p(type = "f")
tiles_2 <- truchet_p(type = "f", scale_p = 1/2)

# Assemble mosaic
mosaic <- st_truchet_ms(tiles_1, tiles_2, prop = 1)
container <- mosaic[["container"]]
mosaic <- mosaic[["mosaic"]]
...
ggsave(filename = "truchet-f-white-on-blue.png", 
       width = 8, 
       height = 12, 
       units = "in")
```
...
```{r}
# Tiles at scale 1
tiles_1 <- rbind(truchet_p(type = "f"),
                 truchet_p(type = "d") %>%
                   mutate(tile = tile + 2))

# Tiles at scale 1/2
tiles_2 <- rbind(truchet_p(scale_p = 1/2, type = "f"),
                 truchet_p(scale_p = 1/2, type = "d") %>%
                   mutate(tile = tile + 2))
...
ggsave(filename = "truchet-f-d-blue-on-white.png", 
       width = 8, 
       height = 12, 
       units = "in")
```
